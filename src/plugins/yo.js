...
function yo(self) {
  ...
  if (
    yohoho.parentNode &&
    yohoho.parentNode.tagName &&
    yohoho.parentNode.tagName.toLowerCase() === "body"
  ) {
    h = Math.max(
    ...
    parseInt(yohoho.offsetHeight) &&
    parseInt(yohoho.offsetHeight) < 370
  ) {
    if (
      yohoho.parentNode &&
    ...
    parseInt(yohoho.offsetHeight) &&
    w / 3 < parseInt(yohoho.offsetHeight)
  ) {
...
function yo_player(iframe, quality, translate, element, buttons, size) {
  ...
  var yohohoButtons = buttons
    ? buttons
    : document.querySelector("#yohoho-buttons");
    ...
    setTimeout(function () {
      var btn = setInterval(function () {
        if (
          ...
          yohohoButtons.style.right =
            parseInt(yohohoButtons.style.right) - 1 + "px";
        } else {
          clearInterval(btn);
        }
      }, 5);
